...
Floating-point is partially supported with either a decimal-float or binary float.

## Status
The core fails to complete the test suite derived from https://github.com/cdifan/cputest. which itself is derived from MicroCoreLabs https://github.com/MicroCoreLabs/Projects/MCL68/MC68000_Test_Code.
Known to be a good test of operation of a 68000. It gets about 3/4 of the way through. So there are some bugs yet.
Almost all instructions are working.
The following instructions are not working correctly: ABCD, SBCD, NBCD, and DIVS.
The procedure to handle BCD numbers is to convert to binary, perform the operation, then convert back to BCD.
Suspect illegal combinations of BCD digits are working differently.
For the divide instructions, they work except that the flags may not be set the same in an overflow state.
Since flags are officially undefined for the overflow state, this should not cause issues with much software.
...
Support for a 68851 similar PMMU is being worked on.

## History
Work started on the core in 2008. It was set aside for a long time, and work resumed in November 2022.
Later resumed in May 2025. Set aside, and resumed again in February 2026.
...
## Size / Performance
Build under Vivado:
approx. 13,000 LUTs (21,000 LC's) without FP / MMU, approx <80 MHz max (in -1 part).
...
## Software
The core may use 68000 software. There is a modified 68k vasm assembler.
Additional control registers were added for movec.
...
### Floating-Point
The core supports 96-bit triple precision decimal floating-point or 96-bit triple precision binary floating-point.
The core repurposes the packed BCD floating-point instructions to implement triple precision densely-packed-decimal floating point.
This may be disabled by commenting out the 'SUPPORT_DECFLT' definition to reduce the core size.
...
opencores.org. OR under the Float repository in Github.
